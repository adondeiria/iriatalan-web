# Knowledge Base (RAG) — Chatbot "María" de WhatsApp

Material de entrenamiento para el chatbot de WhatsApp **María**, asistente de la asesora financiera y de seguros **Iria Talan** (firma RIF), en México.

Este documento consolida **todas las preguntas frecuentes (FAQs) que ya están escritas en las páginas del sitio iriatalan.com.mx**. Las respuestas son **las del sitio** — transcritas fielmente desde los archivos de la web, sin inventar, parafrasear ni "mejorar" contenido. Se limpió el formato JSX a texto plano legible, pero se conservaron las cifras, los nombres de productos y aseguradoras (Allianz, AXA, BUPA, GNP, MetLife, SMNYL, PPR, GMM, Modalidad 40, etc.) y el marco legal exactamente como aparecen.
...
## Empresas (Persona Clave / Hombre Clave)
...
## Personas (Hub de perfiles que asesoro)

### ¿Cuál de los perfiles de Personas aplica a mi caso si no me identifico con ninguno?
...
### ¿Puedo trabajar con Iria si no estoy en ninguno de los perfiles especializados?
...
### ¿Qué hace distinta la asesoría para personas vs para empresas?
...
### ¿Cómo elijo entre asesoría individual y patrimonial?
...
## Mujeres (Asesoría financiera para mujeres)
...
## Hijos neurodivergentes (Familias con hijos con autismo, TDAH, síndrome de Down, etc.)
...
## Foreigners in Mexico (Landing en inglés — extranjeros residentes en México)
...
## Patrimonial (Patrimonios complejos · seguros de vida, fideicomisos, sucesión)
...
### ¿Qué llevo a la primera reunión?
...
### ¿Hay compromiso de contratar después de la consulta?
...
FAQs extraídas por archivo/tema:

| Tema / Landing | Archivo | FAQs |
|---|---|---|
| Inicio (Home) | `src/app/page.tsx` | 5 |
| GMM (Gastos Médicos Mayores) | `src/app/gmm/page.tsx` | 13 |
| Retiro (PPR + Modalidad 40) | `src/app/retiro/page.tsx` | 12 |
| Empresas (Persona Clave) | `src/app/empresas/page.tsx` | 9 |
| Personas (hub de perfiles) | `src/app/personas/page.tsx` | 4 |
| Mujeres | `src/app/personas/mujeres/page.tsx` | 12 |
| Hijos neurodivergentes | `src/app/personas/hijos-neurodivergentes/page.tsx` | 12 |
| Familias arcoíris | `src/app/personas/familias-arcoiris/page.tsx` | 11 |
| Mexicanos en el extranjero | `src/app/personas/mexicanos-en-el-extranjero/page.tsx` | 11 |
| Foreigners in Mexico (inglés) | `src/app/foreigners-in-mexico/page.tsx` | 11 |
| Patrimonial (patrimonios complejos) | `src/app/patrimonial/page.tsx` | 11 |
| Contacto | `src/app/contacto/page.tsx` | 5 |
| **Total** | | **116** |

### Landings revisadas SIN FAQs inline
- `src/app/sobre-iria/page.tsx` — revisada, no contiene array de FAQs.
- `src/app/recursos/page.tsx` — revisada, no contiene array de FAQs.
- `src/app/blog/silencios/page.tsx`, `src/app/blog/page.tsx`, `src/app/glosario/page.tsx`, `src/app/aviso-privacidad/page.tsx` — revisadas, sin FAQs inline.

### Notas
- Las páginas dinámicas (`src/app/[slug]`, `src/app/blog/[slug]`, `src/app/recursos/[slug]`, `src/app/glosario/[slug]`, `src/app/blog/categoria/[slug]`) renderizan contenido desde el CMS Sanity, no FAQs inline en código; no se incluyen aquí.
- En todas las páginas las FAQs tienen una versión en el array `FAQS` (campo `answerText`, texto plano) y otra renderizada en JSX. Se transcribió la versión de `answerText` por ser la fuente canónica limpia. En unos pocos casos el JSX abrevia ligeramente la respuesta del array (p. ej. Retiro "Estoy joven…" y Mujeres "Soy dueña o socia…"); se conservó la versión completa del array.
- El apellido se escribe **Talan** (sin acento) en todo el documento.
