            ...
            <div className="grid gap-x-10 gap-y-12 grid-cols-1 sm:grid-cols-2 lg:grid-cols-5">
              {HELP_CATEGORIES.map((cat) => (
                <div key={cat.title} {...(cat.langEn ? { lang: "en" } : {})}>
                  <h3 className="text-xs uppercase tracking-[0.18em] text-burgundy font-semibold mb-5">
                    {cat.title}
                  </h3>
      ...
      >
        ...
        <div className="relative h-full flex flex-col overflow-y-auto">
          ...
          <nav className="flex-1 px-6 py-8 space-y-10">
            ...
            {HELP_CATEGORIES.map((cat) => (
              <div key={cat.title} {...(cat.langEn ? { lang: "en" } : {})}>
                <h3 className="text-xs uppercase tracking-[0.18em] text-burgundy font-semibold mb-4">
                  {cat.title}
                </h3>
